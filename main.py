...
class Manager:
    ...
    def assign(self, task_name, method):
        self.tasks[task_name] = method

    def execute_task(self, task_name, *args, **kwargs):
        if task_name in self.tasks:
            method = self.tasks[task_name]
    ...
    def balance_decorator(self, func):
        def wrapper(*args, **kwargs):
            result = func(*args, **kwargs)
            print(f"Balance checked: {result}")
            return result
        return wrapper

    def display_inventory_decorator(self, func):
        def wrapper(*args, **kwargs):
            result = func(*args, **kwargs)
            print("Inventory displayed.")
            return result
        return wrapper

    def search_warehouse_decorator(self, func):
        def wrapper(*args, **kwargs):
            result = func(*args, **kwargs)
            print("Warehouse searched.")
            return result
        return wrapper

    def review_history_decorator(self, func):
        def wrapper(*args, **kwargs):
            result = func(*args, **kwargs)
            print("History reviewed.")
            return result
        return wrapper
...
if __name__ == "__main__":
    ...
    manager = Manager(warehouse_system)

    # Assign tasks to methods using decorators
    manager.assign("sale", manager.sale_decorator(warehouse_system.make_sale))
    manager.assign("purchase", manager.purchase_decorator(warehouse_system.make_purchase))
    manager.assign("balance", manager.balance_decorator(warehouse_system.check_balance))
    manager.assign("display_inventory", manager.display_inventory_decorator(warehouse_system.display_inventory))
    manager.assign("search_warehouse", manager.search_warehouse_decorator(warehouse_system.search_warehouse))
    manager.assign("review_history", manager.review_history_decorator(warehouse_system.review_history))

    # Run the manager
    manager.execute_task("sale")
    manager.execute_task("purchase")
    manager.execute_task("balance")
    manager.execute_task("display_inventory")
    manager.execute_task("search_warehouse")
    manager.execute_task("review_history")
